...
export async function publishPost(post) {
  ...
  for (const platform of post.platforms) {
    ...
    if (!tokenInfo || !tokenInfo.accessToken || tokenInfo.accessToken.startsWith('mock_')) {
      const errMsg = `No live access token connected for ${platform.toUpperCase()}. Please configure your page token in Settings.`;
      console.warn(`Scheduler: [MISSING TOKEN] ${errMsg}`);
      publishErrors[platform] = errMsg;
      continue;
    }
    ...
    try {
      if (platform === 'facebook') {
        ...
        // Instagram requires an image attachment to post via the graph API
        if (!post.mediaUrl) {
          throw new Error('Instagram requires an image attachment to publish.');
        }
        
        // Step 1: Create a media container
        const containerUrl = `https://graph.facebook.com/v18.0/${tokenInfo.handle}/media`;
        ...
        // Wait 3 seconds for Instagram processing
        await new Promise(resolve => setTimeout(resolve, 3000));
        
        // Step 2: Publish the media container
        const publishUrl = `https://graph.facebook.com/v18.0/${tokenInfo.handle}/media_publish`;
        ...
        if (!publishRes.ok) {
          throw new Error(publishJson.error?.message || 'Failed to publish Instagram media container');
        }
        
        console.log(`Scheduler: [LIVE SUCCESS] Posted to Instagram! Post ID: ${publishJson.id}`);
        ...
        const err = `Live posting for platform ${platform.toUpperCase()} is not supported.`;
        console.warn(`Scheduler: ${err}`);
        publishErrors[platform] = err;
      }
    } catch (apiErr) {
      console.error(`Scheduler: [LIVE ERROR] Publishing failed for ${platform}:`, apiErr.message);
      publishErrors[platform] = apiErr.message;
    }
  }

  const errorKeys = Object.keys(publishErrors);
  ...
  if (hasErrors) {
    await updatePost(post.id, {
      ...
      publishedAt: null,
      metrics: null,
      errorDetails: JSON.stringify(publishErrors)
    });
    console.error(`Scheduler: Publication failed for post "${post.id}". Errors:`, publishErrors);
    throw new Error(Object.values(publishErrors).join(' | '));
  }
